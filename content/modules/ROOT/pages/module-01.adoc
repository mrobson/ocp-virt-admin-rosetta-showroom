...
Migrating workloads between compute nodes and storage types is a crucial feature of any
modern hypervisor. OpenShift Virtualization allows the sumple and easy process of 
migrating your virtual machine compute and storage from one place to another.
...
[start=2]
. Navigate to Virtualization -> Virtual Machines and then click on the virtual machine named `live-migrate`.
Note which node the VM is currently running on. In this example, it is worker-cluster-m2ssq-1, but your worker node will be different.
...
[start=5]
. Change the selection to Specific Node and then click on the node a node (e.g. `worker-cluster-s67qr-1`. Your lab will have a different node name).
...
Note that the worker node is now worker-cluster-s67qr-1.
...
[start=7]
. A few moments after that, the Virtual Machine status will return to `Running`.
Note that the worker node has changed (e.g. worker-cluster-s67qr-3).
...
[start=1]
. Login to the OpenShift console using the credentials provided to you by the lab administrator. Navigate to Virtualization -> Virtual Machines, and select the virtualmachines project. Select the VM named `live-migrate`.
...
[start=9]
. To view the storage migration status, navigate to Migration -> Storage migration where you can view the status of the migration plan. The migration plan will start in the `Pending` state, then move to `Running`, then finally `Completed`. 
NOTE: It may take some time for the migration to complete.

image::exercise1/01-image-storage-009.png[title="Migration is running", link=self, window=blank, width=100%]

[start=10]
. Once the migration is Complete, navigate back to the virtual machine called `live-migrate` and view the Configuration -> Storage options again.
