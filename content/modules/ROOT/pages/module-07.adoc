...
----
oc login -u {openshift_cluster_admin_username} -p `{openshift_cluster_admin_password}` --server=`{ocp_api_url}`
----
...
. From the left hand menu, navigate to *Migration for Virtualization* and click *Migration plans*.
