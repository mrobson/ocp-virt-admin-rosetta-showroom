...
.. Create a new project for the stress test
[source,bash]
----
oc new-project stress-test
----

.. Create the configuration from a config map
[source,bash]
----
...
[source,bash]
----
oc apply -f https://raw.githubusercontent.com/mmayeras/k8s-stress-test/refs/heads/main/stress-deploy.yaml
----

.. Edit the deployment to use a specific node selector (place it on a node where at least 1 vm is running)

[source,bash]
----
oc patch deployment stress-test --merge -p '{"spec":{"template":{"spec":{"nodeSelector":{"kubernetes.io/hostname":"worker01"}}}}}'
----
