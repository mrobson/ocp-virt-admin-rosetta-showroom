...
+
[,shell,role=execute,subs="attributes"]
----	
ssh {ssh_user}@{ssh_password} -p 32270
----
...
[,shell,role=execute,subs="attributes"]
----
oc login -u {openshift_cluster_admin_username} -p openshift_cluster_admin_password} --server=https://api.cluster-<clustername>.dynamic.redhatworkshops.io:6443
...
The wasp-agent component documentation can be found https://docs.redhat.com/en/documentation/openshift_container_platform/4.20/html/virtualization/postinstallation-configuration#virt-using-wasp-agent-to-configure-higher-vm-workload-density_virt-configuring-higher-vm-workload-density[here].
