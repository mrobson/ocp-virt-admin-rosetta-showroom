...
NOTE: Note that resource assignments are made at virt-launcher pod scheduling time, so any VMs will need to be stopped and restarted to change CPU allocation behavior after a ratio change.
...
**Using the OpenShift Console:**
Navigate to Workloads → Pods and select the virtlauncher pod.
...
[start=1]
. By default, the memory overcommit ratio is 1:1. This means that the amount of memory requested by a VM is equal to the amount of memory allocated to the VM.
...
This overhead is necessary and expected - it ensures the VM has enough resources to run properly.
...
Turn off the VM and restart it to apply the new setting, then observe the memory requests for the virt-launcher pod :
...
[start=4]
. Let's increase the memory overcommit ratio to 200% and add 2GiB to the VM.
...
Stop and restart the VM to apply the new setting, then observe the memory requests for the virt-launcher pod from the web console or from the CLI:
